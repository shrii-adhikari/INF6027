...
#Condition filtering
# Assuming your dataset is named 'news_data' with the following columns:
# - 'headline': The text of the headline
# - 'short_description': The short description text

# Step 1: Calculate the length of each headline and short description
news_data$headline_length <- nchar(news_data$headline)
news_data$short_desc_length <- nchar(news_data$short_description)

# Step 2: Plotting the scatter plot
library(ggplot2)

ggplot(news_data, aes(x = headline_length, y = short_desc_length)) +
  geom_point(alpha = 0.6, colour = "green") +  # Scatter points with slight transparency
  labs(
    title = "Scatter Plot: Headline Length vs. Short Description Length",
    x = "Length of Headlines",
    y = "Length of Short Description"
  ) +
  theme_minimal()
...
write_xlsx(top_10_authors_data, path = file_path)
